    ...
    ui(new Ui::MainWindow)
{
    ...
    ui->TempPlot->addGraph();
    ui->TempPlot->graph(1)->setPen(QPen(Qt::red)); // line color red for first graph (POWER)
    ui->TempPlot->graph(1)->setBrush(QBrush(QColor(255, 0, 0, 0))); // first graph will be filled with translucent blue
    ui->TempPlot->addGraph();
    ui->TempPlot->graph(2)->setPen(QPen(Qt::green)); // line color red for first graph
    ui->TempPlot->graph(2)->setBrush(QBrush(QColor(0, 255, 0, 0))); // seccond graph will be filled with translucent green
...
void MainWindow::RxTimer(){                 //USB Receive timer. DO NOT PLACE MESSAGE BOXES HERE                1ms timer
    ...
    if(TDuC_Comm->isOpen() && TDuC_Comm->isReadable()){
        ...
        if(TDuC_Comm->bytesAvailable()) {
            ...
            qDebug() << "Received: ";
            qDebug() << datos;
            ui->Console_Log->append(datos);
            //ui->Console_Log->setTextCursor()
            //ui->DEBUG_LABEL->setText(ui->DEBUG_LABEL->text()+datos);
            ...
            if(datos.startsWith("T:")){
                ...
                StateValue=more.toInt();                  //Converts State to integer, to switch upon value.

                switch(StateValue) {                                  //Device state value
                case 0:
                    break;
                case 1:                         //Stand By                   
                    ui->statusBar->showMessage("Equipo en Stand By. Presione la perilla principal durante 3 segundos para encenderlo.",0);
                    ui->StartStop->setText("ENCENDER");                 //Start/Stop Button text display change
                    break;
                case 2:                         //Config State
                    flg_StandBy=0;
                    flg_sendExit=0;
                    ...
                    //if(ui->StartStop->isChecked()) {
                        ui->StartStop->setText("COMENZAR");                 //Start/Stop Button text display change
                        ui->pushButton_2->setEnabled(true);                 //TempSet Button enabled
                        ui->pushButton_3->setEnabled(true);                 //TimeSet button enabled
                        ui->TempSet->setEnabled(true);                      //TempSet editBox enabled
                        ui->TimeSet->setEnabled(true);                      //TimeSet editbox enabled
                        ui->StartStop->setChecked(false);                   //Start/Stop Button checked display
                        ui->AutoTunePID->setEnabled(true);                  //Autotune Button enabled
                        ui->ManualTunePID->setEnabled(true);                //ManualTune button enabled
                        ui->TwoPtsCal->setEnabled(true);                    //TwoPoint Calibration button enabled
                        ui->Restore_KyC->setEnabled(true);                  //KyC temperature parameteres restore button enabled
                        ui->Margin_Change->setEnabled(true);                //MarginSetPoint button enabled
                        ui->TwoPtsCal_2->setEnabled(true);                  //TwoPoint Calibration button2 enabled
                        ui->MPtsCal_BT->setEnabled(true);                   //MultiPoint Calibration button enabled
                    //}
...
//                        }
                        ...
                        ui->AutoTunePID->setDisabled(true);                  //Autotune Button Disabled
                        ui->ManualTunePID->setDisabled(true);                //ManualTune button Disabled
                        ui->TwoPtsCal->setDisabled(true);                    //TwoPoint Calibration button Disabled
                        ui->Restore_KyC->setDisabled(true);                  //KyC temperature parameteres restore button Disabled
                        ui->Margin_Change->setDisabled(true);                //MarginSetPoint button Disable
                        ui->TwoPtsCal_2->setDisabled(true);                  //TwoPoint Calibration button2 Disabled
                        ui->MPtsCal_BT->setDisabled(true);                   //MultiPoint Calibration button Disabled
                    //}
                ...
                if(flg_addata) {
                    ...
                    if(flg_savefile){
                        ...
                        if(Archivo.open(QIODevice::Append)) {
                            ...
                            tiempo.sprintf("Tiempo;%ld;;TEMP;",tiempo_archivo);
                            tiempo+= datos + "\n";
                            out << tiempo.toUtf8();
                            //qDebug() << tiempo;
                            Archivo.close();
                    ...
                    flg_addata=0;
                    datos.sprintf("%lld",x_cuenta);
                    //qDebug() << "Plot_addValue X:" + datos;

                    ui->TempPlot->graph(0)->addData(x_cuenta,y_Temp);
                    y_tempSet = ui->TempSet_lb->text().toDouble();
                    ui->TempPlot->graph(2)->addData(x_cuenta,y_tempSet);
                    if(flg_mPtCal) {
            ...
            } else if(datos.startsWith("*DD")){             //receives power data
                r_pos=datos.indexOf("R:");
                datos.remove(0,r_pos+2);
                r_pos=datos.indexOf("___");
                datos=datos.left(r_pos);
                y_tempSet = datos.toDouble();
                ui->TempPlot->graph(1)->addData(x_cuenta,y_tempSet);
                ui->TempPlot->replot();
                ...
                //qDebug() << "K_temp: " + datos;
                valor=datos.toFloat();
                ui->KTemp->setText(QString::number(valor,'f',6));                         //Device info recolection
                if(flg_sendT_K) {
                ...
                //qDebug() << "C_temp: " + datos;
                valor=datos.toFloat();
                ui->CTemp->setText(QString::number(valor,'f',6));                         //Device info recolection
                if(flg_sendT_C) {
            ...
            } else if(datos.startsWith("KTf:")){
                //qDebug() << "Recibe K_temp_fact";
                flg_readT_K_fact=0;
                datos.remove(0,4);
                //qDebug() << "K_temp_fact: " + datos;
                valor=datos.toFloat();
                ui->KTemp_fact->setText(QString::number(valor,'f',6));                         //Device info recolection

                if(flg_sendT_K_fact) {
                    //qDebug() << "Tenía que corregir K_Temp_fact";
                    float comparar;
                    datos=ui->K_temp_fact->text();
                    comparar=datos.toFloat();
                    if(valor==comparar){
                        flg_sendT_K_fact=0;
                        //qDebug() << "Corregido";
                    }
                    //qDebug() << "Confirmado K_temp_fact";
                    cont_sendT_K_fact=0;
                }
                ui->K_temp_fact->setText(QString::number(valor,'f',6));


            } else if(datos.startsWith("CTf:")){
                //qDebug() << "Recibe C_temp_fact";
                flg_readT_C_fact=0;
                datos.remove(0,4);
                //qDebug() << "C_temp_fact: " + datos;
                valor=datos.toFloat();
                ui->CTemp_fact->setText(QString::number(valor,'f',6));                         //Device info recolection

                if(flg_sendT_C_fact) {
                    //qDebug() << "Tenía que corregir C_Temp_fact";
                    float comparar;
                    datos=ui->C_temp_fact->text();
                    comparar=datos.toFloat();
                    if(valor==comparar){
                        flg_sendT_C_fact=0;
                        //qDebug() << "Corregido";
                    }
                    //qDebug() << "Confirmado C_temp_fact";
                    cont_sendT_C_fact=0;
                }
                ui->C_temp_fact->setText(QString::number(valor,'f',6));


            } else if(datos.startsWith("KT2:")){
                //qDebug() << "Recibe K_temp";
                flg_readT_K2=0;
                datos.remove(0,4);
                //qDebug() << "K_temp2: " + datos;
                valor=datos.toFloat();
                ui->KTemp_2->setText(QString::number(valor,'f',6));                         //Device info recolection
                if(flg_sendT_K2) {
                    //qDebug() << "Tenía que corregir K_Temp";
                    float comparar;
                    datos=ui->K_temp_2->text();
                    comparar=datos.toFloat();
                    if(valor==comparar){//ui->TempSet_lb->text()==ui->TempSet->text()){
                        flg_sendT_K2=0;
                        //qDebug() << "Corregido";
                ...
                ui->K_temp_2->setText(QString::number(valor,'f',6));

            } else if(datos.startsWith("CT2:")){
                //qDebug() << "Recibe C_temp";
                flg_readT_C2=0;
                datos.remove(0,4);
                //qDebug() << "C_temp: " + datos;
                valor=datos.toFloat();
                ui->CTemp_2->setText(QString::number(valor,'f',6));                         //Device info recolection
                if(flg_sendT_C) {
                    //qDebug() << "Tenía que corregir C_Temp";
                    float comparar;
                    datos=ui->C_temp_2->text();
                    comparar=datos.toFloat();
                    if(valor==comparar){//ui->TempSet_lb->text()==ui->TempSet->text()){
                        flg_sendT_C2=0;
                        //qDebug() << "Corregido";
                ...
                ui->C_temp_2->setText(QString::number(valor,'f',6));
                ...
                int valor2;
                valor2=datos.toInt();
                ui->Precalentamiento->setText(QString::number(valor,'d'));                         //Device info recolection
                if(flg_sendWarmUp) {
                ...
                int valor2;
                valor2=datos.toInt();
                ui->Safeguard->setText(QString::number(valor,'d'));                         //Device info recolection
                ...
                int valor2;
                valor2=datos.toInt();
                ui->SmartPID->setText(QString::number(valor,'d'));                         //Device info recolection
                if(flg_sendSmart) {
            ...
            } else if(datos.startsWith("Cal2pt_B")){         //Recibió valor de temp
                c_2ptcal_step=3;
                ...
                ui->PID_Kp->setText(QString::number(valor,'f',6));                         //Device info recolection
                if(flg_sendKp) {
                ...
                //qDebug() << "KiPID: " + datos;
                valor=datos.toFloat();
                ui->PID_Ki->setText(QString::number(valor,'f',6));                         //Device info recolection
                if(flg_sendKi) {
                ...
                //qDebug() << "KdPID: " + datos;
                valor=datos.toFloat();
                ui->PID_Kd->setText(QString::number(valor,'f',6));                         //Device info recolection
                if(flg_sendKd) {
                ...
                ui->PID_D->setText(QString::number(valor,'d'));                         //Device info recolection
                ...
                if(valor) {
                    ui->Kd_CheckBox->setCheckState(Qt::CheckState::Checked);
                } else {
                    ui->Kd_CheckBox->setCheckState(Qt::CheckState::Unchecked);
                }
            ...
            } else if(datos.startsWith("NoData")) {                         //Datalog interno - No data
                flg_datalogSave=0;
                flg_readDatalog=0;
                if(ArchivoDatalog.open(QIODevice::Append)) {
                    //qDebug() << "Guarda dato";
                ...
                valor=datos.toFloat() /10;
                ui->MargenDeInicio->setText(QString::number(valor,'f',6));
                if(flg_sendMargin) {
            ...
            } else if(datos.startsWith("Serial Number:")) {                  //SERIAL
                flg_readID=0;
                datos.remove(0,15);
                //qDebug() << "Margin received: " + datos;
                datos=datos.left(datos.length()-2);
                ui->SerialNumber->setText(datos);

            } else if(datos.startsWith("Firmware Version:")) {                  //Version
                flg_readVersion=0;
                datos.remove(0,18);
                //qDebug() << "Margin received: " + datos;
                datos=datos.left(datos.length()-2);
                ui->FWVersion->setText(datos);
...
void MainWindow::mensajesTimeout(){                 //mensajes Timeout (1Hz) para mostrar mensajes en primer plano
    ...
    if(flg_2ptCalMsjA) {
        ...
        c_2ptcal_step++;
        flg_2ptCalMsjA=0;
    } else if(flg_2ptCalMsjB) {
        f_tempA = QInputDialog::getDouble(this,tr("Ingrese temperatura A"),tr("Temperatura A (en °C):"),0);
        if(f_tempA==0.0) {
            QMessageBox::information(this,tr("Calibración de dos puntos"),tr("Se canceló la operación."));
            flg_sendExit=1;
            cont_sendExit=8;
            flg_2ptcal=0;
            c_2ptcal_step=0;
        ...
        QMessageBox::information(this,tr("Calibración de dos puntos"),tr("Por favor, aguarde a que la temperatura se estabilice antes de ingresar el valor de temperatura B."));
        flg_2ptCalMsjB=0;
    } else if (flg_2ptCalMsjC) {
...
void MainWindow::funcionActivacionTimer(){          //100ms timer
    ...
    if(!TDuC_Comm->isOpen()) {
        ...
        if(TDuC_Comm->isReadable()){
            ...
            if(flg_sendExit) cont_sendExit++;

            if(flg_readT_K||flg_sendT_K) cont_sendT_K++;
            if(flg_readT_C||flg_sendT_C) cont_sendT_C++;
            if(flg_readT_K2||flg_sendT_K2) cont_sendT_K2++;
            if(flg_readT_C2||flg_sendT_C2) cont_sendT_C2++;
            if(flg_readT_K_fact||flg_sendT_K_fact) cont_sendT_K_fact++;
            if(flg_readT_C_fact||flg_sendT_C_fact) cont_sendT_C_fact++;
            ...
            if(flg_readID) cont_sendID++;
            if(flg_readVersion) cont_sendVersion++;
            //*************************************
            ...
            //**************************************Read ID
            if(flg_readID&&cont_sendID>10) {
                //qDebug() << "Set Datalog Interval";
                if(TDuC_Comm->isWritable()) {
                    //qDebug() << "adtlgint" + ui->DatalogInterval->value();
                    sprintf(st_datalogTime,"aserial+\n");
                    TDuC_Comm->write(st_datalogTime);
                    cont_sendID=0;
                }
            }
            ...
            //**************************************Read Version
            if(flg_readVersion&&cont_sendVersion>10) {
                //qDebug() << "Set Datalog Interval";
                if(TDuC_Comm->isWritable()) {
                    //qDebug() << "adtlgint" + ui->DatalogInterval->value();
                    sprintf(st_datalogTime,"aversion+\n");
                    TDuC_Comm->write(st_datalogTime);
                    cont_sendVersion=0;
                }
            }
            ...
            //************************************* EXIT command
            if(flg_sendExit&&cont_sendExit>10) {
                //qDebug() << "StartProcess";
                if(TDuC_Comm->isWritable()){
                    //qDebug()<<"astartPr+";
                    TDuC_Comm->write("aexit+\n");
                    cont_sendExit=0;
                }
            }
            //*************************************
            ...
            //******************************Lee o modifica T_K2
            if(flg_readT_K2&&cont_sendT_K2>10) {
                //qDebug() << "Read Temp. K";
                if(TDuC_Comm->isWritable()){
                    //qDebug()<<"aktemp2+";
                    TDuC_Comm->write("aktemp2+\n");
                    cont_sendT_K2=0;
                }
            }
            if(flg_sendT_K2&&cont_sendT_K2>10){
                //qDebug()<< "modifica K_TEMP";
                if(TDuC_Comm->isWritable()){
                    datos=ui->K_temp_2->text();
                    //qDebug() << "aktemp:" + datos;
            ...
            //******************************Lee o modifica T_C2
            if(flg_readT_C2&&cont_sendT_C2>10) {
                //qDebug() << "Read Temp. C";
                if(TDuC_Comm->isWritable()){
                    //qDebug()<<"actemp+";
                    TDuC_Comm->write("actemp2+\n");
                    cont_sendT_C2=0;
                }
            }
            if(flg_sendT_C2&&cont_sendT_C2>10){
                //qDebug()<< "modifica C_TEMP";
                if(TDuC_Comm->isWritable()){
                    datos=ui->C_temp_2->text();
                    //qDebug() << "actemp:" + datos;
            ...
            //******************************Lee o modifica T_K_fact
            if(flg_readT_K_fact&&cont_sendT_K_fact>10) {
                //qDebug() << "Read Temp. K_fact";
                if(TDuC_Comm->isWritable()){
                    //qDebug()<<"aktemp_fact+";
                    TDuC_Comm->write("afktmp+\n");
                    cont_sendT_K_fact=0;
                }
            }
            if(flg_sendT_K_fact&&cont_sendT_K_fact>10){
                //qDebug()<< "modifica K_TEMP_fact";
                if(TDuC_Comm->isWritable()){
                    datos=ui->K_temp_fact->text();
                    //qDebug() << "aktemp:" + datos;
                    TDuC_Comm->write("afktmp,");
            ...
            //******************************Lee o modifica T_C_fact
            if(flg_readT_C_fact&&cont_sendT_C_fact>10) {
                //qDebug() << "Read Temp. C_fact";
                if(TDuC_Comm->isWritable()){
                    //qDebug()<<"afctmp+";
                    TDuC_Comm->write("afctmp+\n");
                    cont_sendT_C_fact=0;
                }
            }
            if(flg_sendT_C_fact&&cont_sendT_C_fact>10){
                //qDebug()<< "modifica C_TEMP";
                if(TDuC_Comm->isWritable()){
                    datos=ui->C_temp_fact->text();
                    //qDebug() << "actemp:" + datos;
    ...
    foreach (const QSerialPortInfo &serialPortInfo, QSerialPortInfo::availablePorts()) {
        ...
        if(serialPortInfo.hasVendorIdentifier()){
            ...
            if((serialPortInfo.vendorIdentifier() == TDuC_VendorID) && (serialPortInfo.productIdentifier() == TDuC_ProductID)){ //&& ((serialPortInfo.serialNumber()=="TDuC")||(serialPortInfo.serialNumber()=="TDUC"))){
                ...
                Puertos.append(serialPortInfo.portName());//serialNumber());
...
void MainWindow::on_StartStop_clicked()
{
    ...
    case 1:                         //Stand By
        ...
        break;
    case 2:                         //Config State
        flg_sendStartPr=1;                  //Start Process
        cont_sendStartPr=9;
        break;
    case 3:                         //Process (heating and regulating)
        flg_sendStopPr=1;                   //Stop Heating Process
        cont_sendStopPr=8;
        break;
    case 4:                         //PIDAT
        flg_sendExit=1;
...
void MainWindow::on_DeviceData_SaveButton_clicked()
{
    QString fileName = QFileDialog::getSaveFileName(this, tr("Seleccionar archivo"),
                                                    QCoreApplication::applicationDirPath(),
                                                    tr("Archivo txt (*.txt)"));
    ui->DeviceData_SaveFile->setText(fileName);
}

void MainWindow::on_DeviceData_Download_clicked()
{
    ArchivoDatalog.setFileName(ui->DeviceData_SaveFile->text());
    if(ArchivoDatalog.open(QIODevice::WriteOnly)){
        //qDebug() << "Genera archivo DatalogInterno";
        ArchivoDatalog.close();
        ArchivoDatalog.setFileName(ui->DeviceData_SaveFile->text());

        ui->DeviceData_SaveFile->setDisabled(true);
        ui->DeviceData_SaveButton->setDisabled(true);

        //************************************************TODO: add routine to save device data to a txt file
        //flg_saveDeviceData=1;
        //cont_sendDatalog=9;
    } else{
        QMessageBox::warning(this,tr("Error de archivo"), tr("El archivo seleccionado no existe"),QMessageBox::Ok);
        //flg_savefile=0;
